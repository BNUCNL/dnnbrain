...
def dnn_activation(input,net,layer,channel=None):
    ...
    Parameters:
    ------------
    input[torch dataloader]: input image dataloader
    net[str]: DNN network
    ...
    dnnact = []
    dnnpicname = []
    for i, (picdata, target, picname) in enumerate(input):
        dnnact_part = actmodel_tmp(picdata)
        dnnact.extend(dnnact_part.detach().numpy())
        dnnpicname += list(picname)
    dnnact = np.array(dnnact)
    dnnpicname = np.array(dnnpicname)
    if channel:
        dnnact = dnnact[:,channel-1,:,:]
    return dnnact, dnnpicname