...
def generate_bold_regressor(X,onset,duration,vol_num,tr):
    ...
    parameters:
    ----------
        X: [n_event] or [n_event,n_sample]
        onset: list or array. in sec. size = n_event 
        duration: list or array. in sec. size = n_event         
        vol_num: int. volume number of BOLD signal.
        tr: in sec
    
	Returns:
    ---------
        X_hrfconvolved_regressor: same shape with X
    '''        
